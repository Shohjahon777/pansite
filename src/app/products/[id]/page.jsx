...
export default function ProductDetailPage() {
  ...
  // Process API product data into component format
  const processProductData = (apiProduct) => {
    // Convert price from string to number (assuming it's in local currency, convert to USD)
    const basePrice = parseInt(apiProduct.price) || 250000
    const usdPrice = Math.round(basePrice / 12500) // Approximate UZS to USD conversion
    ...
    // Process features for "In the box" section
    const boxContents = []
    if (apiProduct.features && apiProduct.features.length > 0) {
      // Handle comma-separated features in a single string or array of features
    ...
    // Process main features from detail_info
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="flex gap-4 mt-4">
                {currentProduct.images.map((image, index) => (
                  ...
                  >
                    ...
                    />
                    <Icon className="w-12 h-12 text-gray-700" strokeWidth={1} style={{ display: 'none' }} />
                  </div>
                ))}
              </div>
            )}
          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-6">
              ...
              <span className="text-gray-400">{currentProduct.rating.toFixed(1)} ({currentProduct.reviewsCount} {t('productDetail.reviews')})</span>
            </div>
          ...
          <div className="py-8">
            ...
            {/* {activeTab === 'reviews' && (
              <div className="space-y-6">
                {currentProduct.reviewsList.map((review, index) => (
                  <div key={index} className="bg-gray-950 border border-gray-800 p-6">
                    ...
                    <p className="text-gray-300 font-light">{review.text}</p>
                  </div>
                ))}
              </div>
            )} */}
